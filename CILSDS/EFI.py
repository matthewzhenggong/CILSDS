...
class ArtificialHorizon(Control):
  ...
  def DrawContent(self):
    ...
    gc.Rotate(-roll_rad)
    gc.Translate(
        (self.AoS * cos(-roll_rad) + self.AoA * sin(-roll_rad)) * scale,
        self.pitch * scale)
    ...
    gc.DrawRectangle(-r - 90 * scale, -r - 90 * scale, 2 * r + 180 * scale,
                     r + 90 * scale)
    ...
    if abs(self.GAMC) < pi / 2:
      ...
      gc.StrokeLine(-w / 10, -self.GAMC * 57.3 * scale, w / 10,
                    -self.GAMC * 57.3 * scale)
...
class EFI(Instrument):
  ...
  def __init__(self, mgr):
    ...
    self.ctrls['BtnFD'] = ButtonSwitch(self, 'FD', ['OFF', 'ON'],
                                       align_right=True)
  ...
  def Layout(self):
    ...
    self.ctrls['V'].SetPosition(x0, margin + ahh / 2 - margin / 2, margin,
                                margin)
    self.ctrls['H'].SetPosition(x0 + margin + ahw,
                                margin + ahh / 2 - margin / 2, margin, margin)
  ...
  def DrawContent(self):
    ...
    gc.DrawText(u'GS{:4.0f}\nM {:4.2f}\nα{:4.1f}\nG {:4.1f}'.format(
        VG, Ma, AoA, GLoad), x0 + 2, margin + margin / 2 + ahh / 2)
    gc.DrawText(u'{:5.0f}'.format(ROC), x0 + margin + ahw + 2,
                margin + margin / 2 + ahh / 2 + 5)
    if AGL > 0:
      gc.DrawText(u'R{:5.0f}'.format(AGL), x0 + margin + ahw + 2,
                  margin + margin / 2 + ahh / 2 + 20)
