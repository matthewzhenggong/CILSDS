...
class Manager(wx.Panel):
  ...
  def __init__(self, parent, log, half=False):
    ...
    self.data = {
        ...
        'NAV': {
            'cur': 1,
            'PNTS': [(23.0, 109.0, 5000), (23.0, 108.5, 5000),
                     (23.2, 108.0, 5000), (22.2, 107.0, 5000)]
        },
        'SENS': {
            'infrWorkStatus': 2,
            'RadarWorkStatus': 3,
  ...
  def Draw(self):
    ...
    gc.StrokeLine(self.cursor[0] - 4, self.cursor[1] - 6, self.cursor[0] + 4,
                  self.cursor[1] - 6)
    gc.StrokeLine(self.cursor[0] - 0.01, self.cursor[1], self.cursor[0] + 0.01,
                  self.cursor[1])
    gc.StrokeLine(self.cursor[0] - 4, self.cursor[1] + 6, self.cursor[0] + 4,
                  self.cursor[1] + 6)
    gc.PopState()
    if self.clicking:
      ...
      gc.StrokeLine(self.cur_pos.x - 50, self.cur_pos.y, self.cur_pos.x + 50,
                    self.cur_pos.y)
      gc.StrokeLine(self.cur_pos.x, self.cur_pos.y - 50, self.cur_pos.x,
                    self.cur_pos.y + 50)
